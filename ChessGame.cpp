...
void ChessGame::setStartPosition(const BasicBoard& bb)
{
...
void ChessGame::getStartPosition(BasicBoard& bb)
{
  if (activePosition<0)
    bb.clear();
  else
    bb.copy(position[0].board);
}
...
bool ChessGame::getPosition(BasicBoard& b)
{
  b.clear();
  if (activePosition<0)
    return false;
...
int ChessGame::isDraw()
{
  ...
  {
    sq=SQUARE128(i);
    switch (position[activePosition].board._board[sq])
    {
...
bool ChessGame::goTo(const BasicBoard& bb, bool strict)
{
